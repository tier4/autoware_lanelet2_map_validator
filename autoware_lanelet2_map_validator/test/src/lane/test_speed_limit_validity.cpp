...
class TestSpeedLimitValidityValidator : public MapValidationTester
{
protected:
  const std::string test_target_ =
    std::string(lanelet::autoware::validation::SpeedLimitValidityValidator::name());
};

TEST_F(TestSpeedLimitValidityValidator, ValidatorAvailability)  // NOLINT for gtest
{
  std::string expected_validator_name =
    lanelet::autoware::validation::SpeedLimitValidityValidator::name();
...
TEST_F(TestSpeedLimitValidityValidator, SampleMap)  // NOLINT for gtest
{
...
TEST_F(TestSpeedLimitValidityValidator, SingleLanelet)  // NOLINT for gtest
{
...
TEST_F(TestSpeedLimitValidityValidator, SpeedLimitWithNegativeValue)  // NOLINT for gtest
{
  load_target_map("lane/speed_limit_with_negative_value.osm");

  lanelet::autoware::validation::SpeedLimitValidityValidator checker;
  const auto & issues = checker(*map_);

  std::map<std::string, std::string> substitution_map;
  substitution_map["speed_limit_value"] = "-10";
...
TEST_F(TestSpeedLimitValidityValidator, SpeedLimitWithOverTheLimitValue)  // NOLINT for gtest
{
  ...
  substitution_map["max_speed_limit"] = "80";
  const auto expected_issue =
