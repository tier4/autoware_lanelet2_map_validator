...
| Area.BufferZoneValidity-003 | Buffer zone polygon must not overlap with {lanelet_subtype} lanelet (ID: {lanelet_id}) unless the lanelet is a road overlapping from a side street. | Warning  | Polygon   | Identifies buffer zones that overlap with road lanelets                                       | Buffer zones must not overlap with road or lanelets (unless it is kind of road that comes from a side street.) |
