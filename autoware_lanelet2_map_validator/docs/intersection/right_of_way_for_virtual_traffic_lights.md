...
| Intersection.RightOfWayForVirtualTrafficLights-007 | Lanelet {soft_conflicting_id} is slightly conflicting with the right_of_way lanelet and there is a chance that this should be a yield lanelet. ({percentage} % of the right_of_way lanelet is covered.) | info     | Regulatory Element | The specified lanelet has a slight chance that it should be set as a yield lanelet for this regulatory element. | Check whether the specified lanelet should be a yield lanelet or not. If it is, add it as a yield lanelet to the regulatory element. |
