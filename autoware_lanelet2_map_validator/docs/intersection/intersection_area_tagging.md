...
3. All lanelets with a `turn_direction` tag that also refer to a `right_of_way` regulatory element have an `intersection_area` tag
...
| Intersection.IntersectionAreaTagging-004 | "This lanelet has turn_direction tag '{turn_direction}' but is missing intersection_area tag."                              | Error    | Lanelet   | The lanelet has a turn_direction tag and refers to a right_of_way regulatory element but is missing the required intersection_area tag | Add an `intersection_area` tag to the lanelet with the appropriate intersection area ID                                     |
