...
from gui_helper import run_lanelet2_validator, DropLineEdit
...
from PySide6.QtWidgets import (
    ...
    QComboBox, QTabWidget, QTextBrowser, QGroupBox, QGridLayout,
...
class ValidatorUI(QMainWindow):
    def __init__(self):
        ...
        # Fetch available validators during initialization
        ...
        # Store tab content to prevent clearing
        self.error_content = ""
        self.warning_content = ""
        
        central = QWidget()
        main_layout = QHBoxLayout(central)  # Top-level horizontal layout
        ...
        # Populate the list with available validators
        ...
        self.validator_edit.setEnabled(False)  # Initially disabled
        ...
        self.error_tab.setOpenExternalLinks(False)  # Prevent default link behavior
        ...
        self.warn_tab.setOpenExternalLinks(False)  # Prevent default link behavior
        ...
        # Input Requirement JSON group
        req_group = QGroupBox("Input Requirement JSON")
        req_layout = QHBoxLayout(req_group)
        self.req_edit = DropLineEdit()
        self.req_edit.setPlaceholderText("Optional: Drag & drop input_requirement.json here or click Browse…")
        ...
        # Parameters YAML group
        param_group = QGroupBox("Parameters YAML")
        param_layout = QHBoxLayout(param_group)
        self.param_edit = DropLineEdit()
        self.param_edit.setPlaceholderText("Optional: Drag & drop params.yaml here or click Browse…")
        ...
        # Exclusion JSON group
        excl_group = QGroupBox("Exclusion List JSON")
        excl_layout = QHBoxLayout(excl_group)
        self.excl_edit = DropLineEdit()
        self.excl_edit.setPlaceholderText("Optional: Drag & drop exclusion_list.json here or click Browse…")
...
----------------------------------------------------------------------------------
        ...
        # Run and Print buttons side by side
        input_panel.addSpacing(18)  # You can adjust the value for more/less space
    ...
    def browse_excl(self):
        fn, _ = QFileDialog.getOpenFileName(self, "Select exclusion list JSON", "", "JSON Files (*.json)")
        if fn:
            self.excl_edit.setText(fn)
            
    def browse_req(self):
        fn, _ = QFileDialog.getOpenFileName(self, "Select input requirement JSON", "", "JSON Files (*.json)")
        if fn:
            self.req_edit.setText(fn)
            
    def browse_param(self):
        fn, _ = QFileDialog.getOpenFileName(self, "Select Parameters YAML", "", "YAML Files (*.yaml)")
        if fn:
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "autoware_lanelet2_map_validator", "--print"
                ],
            ...
            # Parse the output to extract validator names
            validators = []
            if output:
                lines = output.split('\n')
                for line in lines:
                    line = line.strip()
                    # Skip empty lines, lines starting with =, -, or common words
                    if (line and 
                        not line.startswith('=') and 
                        not line.startswith('-') and
                        not line.startswith('Available') and
                        not line.startswith('The') and
                        not line.lower().startswith('list') and
                        not line.lower().startswith('following') and
                        len(line.split()) > 0):
                        
                        # Extract validator name (usually the part before any description)
                        validator_name = line.split()[0] if line.split() else line
                        
                        # Additional filtering: validator names typically contain dots or underscores
                        # and don't start with common English words
                        if (validator_name and 
                            validator_name not in validators and
                            validator_name not in ['The', 'Available', 'List', 'Following', 'validators:', 'are:'] and
                            not validator_name.lower() in ['the', 'available', 'list', 'following', 'validators', 'are'] and
                            ('.' in validator_name or '_' in validator_name or validator_name.islower())):
    ...
    def toggle_manual_input(self, state):
        ...
        if is_manual:
            # Clear list selection when switching to manual
            self.validator_list.clearSelection()
        else:
            # Clear manual input when switching to picker
    ...
    def run_validator(self):
        ...
        if not os.path.exists(osm_path):
            self.error_content = '<span style="color:red">Please specify valid .osm file first.</span>'
        ...
        if req_path and not os.path.exists(req_path):
            self.error_content = '<span style="color:red">Input requirement file path is invalid.</span>'
            self.error_tab.setHtml(self.error_content)
            self.warn_tab.clear()
            self.tab_widget.setCurrentWidget(self.error_tab)
            return
        ...
        try:
            # Load the OSM file into the map visualizer before running validation
            try:
                # Use the same projector type as selected in the options
            ...
            if rc != 0 or err_lines:
                if rc != 0:
                    error_html.append('<span style="color:red">Validator failed or errors present:</span><br/>')
                for line in err_lines:
                    if ':' in line:
                        _, rest = line.split(':', 1)
                        clickable_rest = self.make_clickable_message(rest)
                        error_html.append(f'<span style="color:red">Error:</span><span>{clickable_rest}</span><br/>')
                    else:
                        clickable_line = self.make_clickable_message(line)
                        error_html.append(f'<span style="color:red">Error:</span><span>{clickable_line}</span><br/>')
            ...
            for line in out_lines:
                if line.lower().startswith('warning'):
                    if ':' in line:
                        _, rest = line.split(':', 1)
                        clickable_rest = self.make_clickable_message(rest)
                        warn_html.append(f'<span style="color:orange">Warning:</span><span>{clickable_rest}</span><br/>')
                    else:
                        clickable_line = self.make_clickable_message(line)
                        warn_html.append(f'<span style="color:orange">Warning:</span><span>{clickable_line}</span><br/>')
            
            # Set warning content and store it
            self.warning_content = ''.join(warn_html) if warn_html else '<span>No warnings.</span>'
            self.warn_tab.setHtml(self.warning_content)
            
            # Default to Errors tab if errors, else Warnings
            if error_html:
                self.tab_widget.setCurrentWidget(self.error_tab)
            else:
            ...
            if output_dir:
                ...
                if existed_before:
                    msg = f'<span>[{now}] lanelet2_validation_results.json <span style="color:yellow"> overwritten </span><span> in {output_dir}</span>'
                else:
                    msg = f'<span>[{now}] lanelet2_validation_results.json <span style="color:green"> saved </span><span> to {output_dir}</span>'
                self.info_tab.append(msg)
                                
        except Exception as e:
            self.error_content = f'<span style="color:red">Error running validator:<br/>{e}</span>'
            self.error_tab.setHtml(self.error_content)
            self.warn_tab.clear()
            self.tab_widget.setCurrentWidget(self.error_tab)
    ...
    def make_clickable_message(self, message: str) -> str:
        ...
        # Pattern to match element IDs in validation messages
        # More comprehensive patterns to catch any element type with ID
        patterns = [
            # Common capitalized patterns
            ...
            (r'linestring\s+(\d+)', 'LineString'),
            (r'point\s+(\d+)', 'Point'),
            # Additional patterns for other element types
        ...
        for pattern, element_type in patterns:
            def replace_func(match):
                ...
                return f'<a href="element://{element_type}/{element_id}" style="color: #0066cc; text-decoration: underline;">{match.group(0)}</a>'
            result = re.sub(pattern, replace_func, result, flags=re.IGNORECASE)
        
        return result
    
    def setup_text_edit_links(self, text_edit: QTextBrowser):
        """Setup clickable links in a text edit widget"""
        # Store the content before link click to prevent it from being cleared
        def on_anchor_clicked(url):
            # Save the current content based on which tab this is
